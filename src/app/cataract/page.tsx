...
//       await appendChatMessage(session?.user?.email ??  "","system",resTranscript,transcript);
//     }
//     else if (userInput) {
...
//   const predictVideoText = async (file: File): Promise<string> => {
...
import axios from 'axios';
import './index.css';
import { appendChatMessage } from './functions/appendChatMessage';
import { simulateResponse } from './functions/simulateResponse';
import { processVideo } from './functions/processVideo';
import { predictVideoText } from './functions/predictVideoText';
...
const Chatbot: React.FC = () => {
  ...
  const [videoFile, setVideoFile] = useState<File | null>(null);
  const [transcript, setTranscript] = useState<string>("");
  const [dateTimeId, setDateTimeId] = useState<string>("");
  ...
  const handleSendMessage = async () => {
    ...
    if (videoFile) {
      const videoData = await processVideo(videoFile);
      const resTranscript = await predictVideoText(videoFile);
      await appendChatMessage(session?.user?.email ?? "", "user", "Video Input",dateTimeId, "");
      setTranscript(resTranscript);
      newMessages.push({ type: 'user', message: 'Sent a video', videoSource: videoData }, { type: 'system', message: resTranscript });
      await appendChatMessage(session?.user?.email ?? "", "system", resTranscript, dateTimeId, transcript);
    } else if (userInput) {
      await appendChatMessage(session?.user?.email ?? "", "user", userInput, dateTimeId, "");
      const response = await simulateResponse(userInput, transcript);
      newMessages.push({ type: 'user', message: userInput }, { type: 'system', message: response });
      await appendChatMessage(session?.user?.email ?? "", "system", response, dateTimeId,transcript);
    }
