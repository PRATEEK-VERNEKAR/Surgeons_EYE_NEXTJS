...
import { appendChatMessage } from '@/app/video_text_qa/hysterectomy/functions/appendChatMessage';
import { simulateResponse } from '@/app/video_text_qa/hysterectomy/functions/simulateResponse';
import { processVideo } from '@/app/video_text_qa/hysterectomy/functions/processVideo';
import { predictVideoText } from '@/app/video_text_qa/hysterectomy/functions/predictVideoText';
...
const ChatBody: React.FC<PageProps> = (props:PageProps) => {
  ...
  const handleSendMessage = async () => {
    ...
    if (videoFile) {
      ...
      await appendChatMessage(session?.user?.email ?? "", "user", "Video Input", dateTimeId, "",'uterus');
      newMessages.push({ type: 'user', message: 'Sent a video', videoSource: videoData }, { type: 'system', message: resTranscript });
      await appendChatMessage(session?.user?.email ?? "", "system", resTranscript, dateTimeId, resTranscript,'uterus');
    } else if (userInput) {
      await appendChatMessage(session?.user?.email ?? "", "user", userInput, dateTimeId, "",'uterus');
      ...
      await appendChatMessage(session?.user?.email ?? "", "system", response, dateTimeId, "",'uterus');
    }
    ...
    <div className='flex w-full '>
      <div className="flex flex-col h-[800px] w-[100%] mx-2 rounded-xl bg-gray-200">
        <div className='flex w-full bg-red-200 font-bold justify-center'>
          <h1 className='text-center text-4xl p-2 rounded-xl mr-[12px]'>HYSTERECTOMY</h1>
